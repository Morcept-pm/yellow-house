...
import { listAllNewsAdmin, deleteNews } from "../../lib/content/news";
...
export function NewsList() {
  ...
  const load = () => listAllNewsAdmin().then(setItems);
  useEffect(() => {
    load();
  }, []);
  ...
  return (
    <AdminShell>
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-headline-lg text-2xl text-primary font-medium">最新消息</h1>
        <Link href="/admin/news/new" className="rounded-md px-5 py-3 bg-primary text-white text-sm font-medium hover:bg-primary/90 transition-colors">
          ＋ 新增消息
        </Link>
      </div>

      {items === null && <p className="text-sm text-on-surface-variant">載入中...</p>}
      {items?.length === 0 && <p className="text-sm text-on-surface-variant">尚無任何消息，點擊右上角新增。</p>}
        ...
        {items?.map((item) => (
          <div key={item.id} className="rounded-lg bg-surface-container-lowest border border-outline-variant p-5 flex items-center gap-4">
            ...
            <div className="flex gap-2 shrink-0">
              <Link href={`/admin/news/${item.id}`} className="rounded-md px-4 py-2 text-xs border border-outline-variant hover:border-primary transition-colors">
                編輯
              </Link>
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (pendingDelete) await deleteNews(pendingDelete.id);
